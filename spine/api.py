...
class SpineAPI(View):
    ...
    @property
    def data(self, decoder_prefix='_get_data_for_'):
        """
        ...
        if self._data is None:
            if self.content_type == 'application/json':
                ...
                try:
                    data = getattr(self, decoder_name)()
                    if self.page_param_name in data:
                        self._page_number = data.pop(self.page_param_name)
                    self._data = data
                except ValueError:
                    return BadRequest()
            else:
    ...
    def _get_data_for_get(self):
        ...
        """
        return self.json_decoder.decode(self.request.GET.keys() or '{}')

    def _get_data_for_post(self):
        """
        Decodes the data for POST and PUT requests
        """
        return self.json_decoder.decode(self.request.raw_post_data)
    ...
    def paginate(self, queryset):
        ...
        if (isinstance(queryset, (QuerySet, list, tuple))
            ...
            # this seems very dumb but here the page_number is setted
            self.data

            offset = (self._page_number - 1) * self.page_size
